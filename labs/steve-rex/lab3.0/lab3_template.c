...
#warning "Unimplemented function: char read_shaft_encoder(void)"	// delete this line after implementing
	...
	static unsigned char old_value = 10;		// Step 2: Based on how you decided to mask PORTB in step 1, what would new_value be if the switch is in a groove?

	// Function variables
	unsigned char new_value = 10;			// Step 1: Decide how to read PINC so that the push buttons are masked
	int8_t rotation = 0;

	// If the knob was in a groove MIGHT BE REVERSED!!!!!!!!!
	if (old_value == 10) {			// Step 3: Enter here what you did for step 2.
		if (new_value == 10)		// What will new_value be if you're rotating CW out of a groove
			rotation = 1;
		if (new_value == 10)		// What will new_value be if you're rotating CCW out of a groove
			rotation = -1;
	}
