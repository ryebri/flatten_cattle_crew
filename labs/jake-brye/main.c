...
//	sprintf(send_string, "number of objects found %d", obj_count);
	...
	sprintf(send_string, "{\"position\": [000,000,000,000,050]}\n\0");
	uart_sendStr(send_string);
	timer_waitMillis(50);
	uart_flush();

	sprintf(send_string, "{\"sensors\": [000,000,000,000,001]}\n\0");
	uart_sendStr(send_string);
	uart_flush();
...
void analyze()
{
	...
	sprintf(send_string, "{\"ir\": [");
		...
		for(i = 0; i < 180; i++)
		{
			temp = ir_measure[i]/10;
			if(temp == 0){
				send_string[j] = '0';
				j++;
				send_string[j] = '0';
				j++;
				send_string[j] = (char)(ir_measure[i] + '0');
				j++;
				...
				j++;
				send_string[j] = (char)(ir_measure[i]%10 + '0');
				j++;

			} else {
				send_string[j] = (char)(ir_measure[i]/100 + '0');
				j++;
				temp = ir_measure[i]%100;
...
void find_width()
{
	...
	for(i = 0; i < obj_count; i++)
	{
		...
		if(obj[i].distance < 80 && obj[i].distance > 10)
		{
//			obj[i].width = sqrt(((ir_measure[(obj[i].start_degree)])*(ir_measure[(obj[i].start_degree)]))+(ir_measure[(obj[i].end_degree)])*(ir_measure[(obj[i].end_degree)]) - 2*(ir_measure[(obj[i].start_degree)])*(ir_measure[(obj[i].end_degree)])*cos((degree)));
		...
		//		sprintf(send_string, "Object degree %d\t distance: %d\n");
				...
				send_string[j] = (char)(obj[i].distance/100 + '0');
				j++;
				temp = obj[i].distance%100;
				send_string[j] = (char)(temp/10 + '0');
				j++;
				send_string[j] = (char)(temp%10 + '0');
				j++;
				...
				send_string[j] = (char)(((degree/2) + obj[i].start_degree)/100 + '0');
				j++;
				temp = ((degree/2) + obj[i].start_degree)%100;
				send_string[j] = (char)(temp/10 + '0');
				j++;
				send_string[j] = (char)(temp%10 + '0');
				j++;
				...
				send_string[j] = (char)(obj[i].width/100 + '0');
				j++;
				temp = obj[i].width%100;
				send_string[j] = (char)(temp/10 + '0');
				j++;
				send_string[j] = (char)(temp%10 + '0');
				j++;
